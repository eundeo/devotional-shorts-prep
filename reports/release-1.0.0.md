...
이 문서는 2026-08-13의 `1.0.0` 릴리스 검증 기록이다. 최신 배포본은 [1.0.1 배포 준비 보고서](release-1.0.1.md), 현재 사용 조건은 [운영 보완 결과](operations-readiness-2026-09-08.md)를 따른다.

- 정식 ZIP 배포: 완료
- 다른 사용자 설치 가능 상태: 패키지 검증 완료, 사용자별 설정·외부 인수 필요
...
- 첫 릴리스 커밋 `12757c6`은 생성되었다. 2026-09-08 재점검 기준 태그·GitHub 원격·외부 릴리스는 없다.
- 당시 임시 경로의 종단간 인수 작업 원본은 현재 확인되지 않는다. 이 문서의 과거 검증 기록을 현재 원본 재검증 결과로 해석하지 않는다.
