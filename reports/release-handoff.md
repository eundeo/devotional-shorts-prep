...
플러그인 `1.0.1`의 내부 수정, 자동 검사, 신규 설치, 기존 `1.0.0` 업데이트, 배포 ZIP 준비가 완료됐다. 다른 사용자는 ZIP과 동봉 매니페스트로 설치할 수 있다. 실제 Telegram 운영은 각 사용자의 환경변수와 테스트 그룹 인수가 필요하며, GitHub 공개 게시에는 대상 원격 저장소와 공개 범위 지정이 필요하다.
