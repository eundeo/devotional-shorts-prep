...
`getMe`, `getChat`, `getChatMember`만 호출하며 테스트 메시지를 보내지 않는다. 봇·그룹 접근, 명시적으로 확인 가능한 메시지·문서·사진 권한, 담당자 수를 확인한다. API가 권한을 완전히 확정하지 못하면 실제 전송 단계에서 다시 확인한다.
...
- API의 429·5xx·네트워크 실패는 대기 없이 한 번만 즉시 재시도한다.
